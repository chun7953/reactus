...
async function createTables(db) {
    ...
    await db.query(`CREATE TABLE IF NOT EXISTS calendar_post_assets ( id UUID PRIMARY KEY, guild_id TEXT NOT NULL, filename TEXT NOT NULL, content_type TEXT NOT NULL, size_bytes INTEGER NOT NULL, data BYTEA NOT NULL, created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP NOT NULL, calendar_id TEXT, event_id TEXT, last_verified_at TIMESTAMP WITH TIME ZONE, last_checked_at TIMESTAMP WITH TIME ZONE, missing_since TIMESTAMP WITH TIME ZONE );`);
    ...
    await db.query(`ALTER TABLE calendar_post_assets ADD COLUMN IF NOT EXISTS calendar_id TEXT;`);
    await db.query(`ALTER TABLE calendar_post_assets ADD COLUMN IF NOT EXISTS event_id TEXT;`);
    await db.query(`ALTER TABLE calendar_post_assets ADD COLUMN IF NOT EXISTS last_verified_at TIMESTAMP WITH TIME ZONE;`);
    await db.query(`ALTER TABLE calendar_post_assets ADD COLUMN IF NOT EXISTS last_checked_at TIMESTAMP WITH TIME ZONE;`);
    await db.query(`ALTER TABLE calendar_post_assets ADD COLUMN IF NOT EXISTS missing_since TIMESTAMP WITH TIME ZONE;`);
    await db.query(`CREATE INDEX IF NOT EXISTS calendar_post_assets_owner_idx ON calendar_post_assets (guild_id, calendar_id, event_id) WHERE calendar_id IS NOT NULL AND event_id IS NOT NULL;`);
    await db.query(`CREATE INDEX IF NOT EXISTS calendar_post_assets_reconcile_idx ON calendar_post_assets (guild_id, last_checked_at, last_verified_at) WHERE calendar_id IS NOT NULL AND event_id IS NOT NULL;`);
