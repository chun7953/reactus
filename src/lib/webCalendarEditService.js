...
import { deleteCalendarPostImage, storeCalendarPostImageBuffer } from './calendarPostAssets.js';
...
async function resolveEvent(guildId, { calendarId, eventId, scope = 'instance' }) {
    ...
    if (!['instance', 'series'].includes(scope)) throw new Error('編集範囲が正しくありません。');
    ...
    if (scope === 'series' && source.recurringEventId) {
        try {
            target = (await calendar.events.get({ calendarId, eventId: source.recurringEventId })).data;
        } catch (error) {
            if (error?.code === 403) throw new Error(permissionHelp(auth, calendarId));
    ...
    return {
        ...
        target,
        monitor,
        originalWasRecurring: Boolean(source.recurringEventId || source.recurrence?.length || target.recurrence?.length),
    };
}
...
export async function getWebScheduleDetail(guildId, request) {
    ...
    return {
        ...webScheduleDetail(resolved.target, resolved.monitor),
...
export async function updateWebSchedule(guildId, payload) {
    ...
    const recurrenceAllowed = !resolved.originalWasRecurring || scope === 'series';
    ...
    if (resolved.originalWasRecurring && scope !== 'series' && imageMode !== 'keep') {
        throw new Error('定期予定の画像変更は「繰り返し全体」で編集してください。');
    }
    ...
    try {
        ...
        if (targetType === 'post') {
            ...
            const trigger = cleanKeyword(resolved.monitor.trigger_keyword);
            requestBody = {
                ...
                ...(recurrenceAllowed ? { recurrence: recurrence || [] } : {}),
            };
        } else {
