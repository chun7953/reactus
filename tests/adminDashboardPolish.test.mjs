...
test('beginner guide puts the month calendar before creation and settings actions', async () => {
  const source = await readFile(guidePath, 'utf8');
  const calendar = source.indexOf("title: '月カレンダーを見る'");
  const schedule = source.indexOf("title: '予定・抽選を作る'");
  const settings = source.indexOf("title: 'Googleカレンダー・投稿先を設定する'");
  assert.ok(calendar >= 0 && schedule > calendar && settings > schedule);
});
